...
def load_model(checkpoint=None, warm_start_zhtw=False, model_id=None):
    ...
    if warm_start_zhtw:
        # Revive the untrained zh-TW slot (5) by copying the working zh-CN column (4)
        # of the prompt projector — lossless (zh-TW slot then == zh-CN); s2t for output.
        with torch.no_grad():
            w = m.prompt_projector.linear_1.weight  # [intermediate, 1024+128]
            w[:, 1024 + 5].copy_(w[:, 1024 + 4])
    return m
