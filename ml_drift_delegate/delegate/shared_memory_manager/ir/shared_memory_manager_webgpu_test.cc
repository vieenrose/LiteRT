...
#include "ml_drift_delegate/delegate/shared_memory_manager/ir/shared_memory_manager_webgpu.h"
...
class SharedMemoryManagerTest : public webgpu::WebGpuOperationTest {};
...
TEST_F(SharedMemoryManagerTest, CreateExternalFloatTensorF16FromF32) {
  TensorIdToSharedTensorMap buffer_id_to_spatial_tensor;
  TensorIdToSharedTensorMap param_tensors;
  ...
  ir::IrModel graph;
  ir::IrOp* node = graph.add_op();
  ir::IrTensor* input = graph.add_tensor(DataType::FLOAT32, BHWC(1, 1, 1, 10));
  ir::IrTensor* output = graph.add_tensor(DataType::FLOAT32, BHWC(1, 1, 1, 10));
  graph.AddConsumer(input->id, node->id);
  graph.SetProducer(output->id, node->id);
  ...
  absl::flat_hash_map<ir::IrTensorId, SharedMemoryManager::GlobalId>
...
TEST_F(SharedMemoryManagerTest, CreateExternalFloatTensorF32FromF16) {
  ...
  ir::IrModel graph;
  ir::IrOp* node = graph.add_op();
  ir::IrTensor* input = graph.add_tensor(DataType::FLOAT16, BHWC(1, 1, 1, 10));
  ir::IrTensor* output = graph.add_tensor(DataType::FLOAT16, BHWC(1, 1, 1, 10));
  graph.AddConsumer(input->id, node->id);
  graph.SetProducer(output->id, node->id);
...
TEST_F(SharedMemoryManagerTest, CreateExternalQuantizedTensor) {
  ...
  // Emulate the call from the second signature. In the IrModel world each
  // signature is a distinct subgraph, so the shared weight is referenced by a
  // fresh local tensor id (all mapping to the same global id).
  ir::IrOp* node2 = graph.add_op();
  ir::IrTensor* input2 = graph.add_tensor(DataType::FLOAT32, BHWC(1, 1, 1, 10));
  graph.AddConsumer(input2->id, node2->id);
  local_to_global_id_map.clear();
  ASSERT_OK(manager->RegisterExternalConstantTensors(
...
void RunBlockwiseQuantizationTest(const webgpu::ExecutionEnvironment& env,
  ...
  ir::IrModel graph;
  ir::IrOp* node = graph.add_op();
  node->name = "fully_connected";
  ir::IrTensor* weights_val =
      graph.add_tensor(DataType::FLOAT32, BHWC(100, 1, 1, 64));
  graph.AddConsumer(weights_val->id, node->id);
...
TEST_F(SharedMemoryManagerTest,
  ...
  RunBlockwiseQuantizationTest(exec_env_.GetEnv(), kTfLiteInt32, 200,
                               expected_zp);
}
...
TEST_F(SharedMemoryManagerTest,
  ...
  std::vector<int> expected_zp(200, 5);
  RunBlockwiseQuantizationTest(exec_env_.GetEnv(), kTfLiteInt64, 200,
                               expected_zp,
...
TEST_F(SharedMemoryManagerTest,
  ...
  std::vector<int> expected_zp(200, 5);
  RunBlockwiseQuantizationTest(exec_env_.GetEnv(), kTfLiteInt8, 200,
                               expected_zp,
                               /*use_invalid_zp=*/false,
                               /*expect_failure=*/true);
}
...
TEST_F(SharedMemoryManagerTest,
  ...
  RunBlockwiseQuantizationTest(exec_env_.GetEnv(), kTfLiteInt32, 1,
                               expected_zp);
}
