...
#ifndef THIRD_PARTY_ODML_LITERT_ML_DRIFT_DELEGATE_SHARED_MEMORY_MANAGER_IR_SHARED_MEMORY_MANAGER_METAL_H_
#define THIRD_PARTY_ODML_LITERT_ML_DRIFT_DELEGATE_SHARED_MEMORY_MANAGER_IR_SHARED_MEMORY_MANAGER_METAL_H_
...
inline std::unique_ptr<SharedMemoryManager> MakeSharedMemoryManagerMetal(
    metal::MetalDevice* device, const CreateGpuModelInfo& create_info, ir::IrModel& graph,
    TfLiteContext* context, TensorIdToSharedTensorMap& value_to_tensor_map,
    TensorIdToSharedTensorMap& quant_param_tensors, bool has_prepacked_external_tensors,
  ...
  return std::make_unique<SharedMemoryManager>(
      ...
      std::move(maybe_bind_data), std::move(packing_lookup), std::move(discard_tensor_data));
}

}  // namespace ir
}  // namespace ml_drift

#endif  // THIRD_PARTY_ODML_LITERT_ML_DRIFT_DELEGATE_SHARED_MEMORY_MANAGER_IR_SHARED_MEMORY_MANAGER_METAL_H_
