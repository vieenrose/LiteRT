...
#include "ml_drift_delegate/delegate/shared_memory_manager/ir/shared_memory_manager_cl.h"
...
TEST_F(SharedMemoryManagerTest, CreateExternalFloatTensorF16FromF32) {
  ...
  ir::IrTensor* input = graph.add_tensor(ml_drift::TensorDescriptor{});
  ir::IrTensor* output = graph.add_tensor(ml_drift::TensorDescriptor{});
  input->consumers.insert(op->id);
  op->inputs.push_back(input->id);
  output->producer = op->id;
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  ...
  absl::flat_hash_map<ir::IrTensorId,
                      ml_drift::ir::SharedMemoryManager::GlobalId>
  ...
  ir::IrOp* op2 = graph.add_op();
  ir::IrTensor* input2 = graph.add_tensor(ml_drift::TensorDescriptor{});
  input2->consumers.insert(op2->id);
  op2->inputs.push_back(input2->id);
  input2->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  ASSERT_OK(manager->RegisterExternalConstantTensors(
      input2->id, shared_tflite_tensor, local_to_global_id_map));
...
// Regression test for shared-constant registration. The descriptor is mutated
// in place (matching GraphFloat32), so the tensor id is stable: the consumer op
// keeps referencing the same id, the tensor is not tombstoned, its descriptor
// is updated (dtype), `local_to_global_id_map` stays keyed on that stable id,
// and a second signature reuses the same GPU tensor.
TEST_F(SharedMemoryManagerTest, SharedConstantIsMutatedInPlaceWithStableId) {
  TensorIdToSharedTensorMap buffer_id_to_spatial_tensor;
  TensorIdToSharedTensorMap quant_param_tensors;
  ...
  ir::IrModel graph;
  ir::IrOp* op = graph.add_op();
  ir::IrTensor* weights = graph.add_tensor(ml_drift::TensorDescriptor{});
  ir::IrTensor* output = graph.add_tensor(ml_drift::TensorDescriptor{});
  weights->consumers.insert(op->id);
  op->inputs.push_back(weights->id);
  output->producer = op->id;
  op->outputs.push_back(output->id);
  weights->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  const ir::IrTensorId original_weights_id = weights->id;
  ...
  // ---- First signature: creates the shared GPU tensor. ----
  absl::flat_hash_map<ir::IrTensorId,
                      ml_drift::ir::SharedMemoryManager::GlobalId>
      local_to_global_id_map;
  ASSERT_OK(manager->RegisterExternalConstantTensors(
  ...
  // The tensor is mutated in place: the consumer op still references the same
  // id, the tensor is still alive, and its descriptor was updated to FLOAT16.
  ASSERT_EQ(op->inputs.size(), 1);
  EXPECT_EQ(op->inputs[0], original_weights_id);
  ASSERT_NE(graph.tensor(original_weights_id), nullptr);
  EXPECT_EQ(graph.tensor(original_weights_id)->desc.GetDataType(),
            DataType::FLOAT16);

  // The id map is keyed on the stable id, so a lookup by consumer input
  // resolves to the shared GPU tensor.
  ASSERT_EQ(local_to_global_id_map.size(), 1);
  ...
  // ---- Second signature: reuses the GPU tensor with a fresh local id. ----
  local_to_global_id_map.clear();
  ir::IrOp* op2 = graph.add_op();
  ir::IrTensor* weights2 = graph.add_tensor(ml_drift::TensorDescriptor{});
  weights2->consumers.insert(op2->id);
  op2->inputs.push_back(weights2->id);
  weights2->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  const ir::IrTensorId original_weights2_id = weights2->id;

  ASSERT_OK(manager->RegisterExternalConstantTensors(
      original_weights2_id, shared_tflite_tensor, local_to_global_id_map));
  // No new GPU tensor is allocated on reuse.
  EXPECT_EQ(buffer_id_to_spatial_tensor.size(), 1);

  ASSERT_EQ(op2->inputs.size(), 1);
  EXPECT_EQ(op2->inputs[0], original_weights2_id);
  ASSERT_EQ(local_to_global_id_map.size(), 1);
  ASSERT_TRUE(local_to_global_id_map.contains(original_weights2_id));
  ASSERT_OK_AND_ASSIGN(ml_drift::GpuSpatialTensor * external_tensor2,
                       manager->GetExternalConstantTensor(
                           local_to_global_id_map.at(original_weights2_id)));
  // Both signatures resolve to the same shared GPU tensor.
  EXPECT_EQ(external_tensor2, external_tensor);
...
TEST_F(SharedMemoryManagerTest, CreateExternalFloatTensorF32FromF16) {
  ...
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  input->desc.SetDataType(DataType::FLOAT16);
...
TEST_F(SharedMemoryManagerTest, CreateExternalFloatTensorF32FromF32) {
  ...
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  input->desc.SetDataType(DataType::FLOAT32);
...
TEST_F(SharedMemoryManagerTest, CreateQuantizedTensorMixedPrecision) {
  TensorIdToSharedTensorMap buffer_id_to_spatial_tensor;
  TensorIdToSharedTensorMap quant_param_tensors;
  ...
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(BHWC(1, 1, 1, 10));
  input->desc.SetDataType(DataType::FLOAT16);  // input is fp16
...
void RunBlockwiseQuantizationTest(cl::Environment& env, TfLiteType zp_type,
                                  int zp_size,
  ...
  ir::IrTensor* weights_val = graph.add_tensor(ml_drift::TensorDescriptor{});
  weights_val->consumers.insert(op->id);
  op->inputs.push_back(weights_val->id);
  weights_val->desc.SetBHWCShape(BHWC(100, 1, 1, 64));
...
TEST_F(SharedMemoryManagerTest,
  ...
  std::vector<int> expected_zp(200, 5);
  RunBlockwiseQuantizationTest(env_, kTfLiteInt64, 200, expected_zp,
...
TEST_F(SharedMemoryManagerTest,
  ...
  std::vector<int> expected_zp(200, 5);
  RunBlockwiseQuantizationTest(env_, kTfLiteInt8, 200, expected_zp,
                               /*use_invalid_zp=*/false,
                               /*expect_failure=*/true);
}
...
TEST_F(SharedMemoryManagerTest,
  ...
  RunBlockwiseQuantizationTest(env_, kTfLiteInt32, 1, expected_zp);
}
