...
// Do not enable madvise on Web or Windows because it has not been tested.
  ...
  MadviseData(original_ptr, space);
...
bool IsConvInt8Supported(const CreateGpuModelInfo& create_info,
                         const GpuInfo& gpu_info, const ir::IrModel& graph,
                         const ir::IrOp* fc_node) {
  if (create_info.hints.Check(ModelHints::kDisallow8bitConvs)) {
    return false;
  }

  const ir::IrTensor* src_tensor = graph.tensor(fc_node->inputs[0]);
  ...
  return supports_conv_apple_mpp ||
         SupportsConvWaveMatrixMaliInt8(gpu_info,
                                        src_tensor->desc.GetBHWCShape()) ||
         SupportsConvWaveMemoryInt8(gpu_info) ||
         SupportsConvGenericInt8(gpu_info) ||
         SupportsConvWaveMatrixInt8(
             gpu_info, OHWI(32, 1, 1, src_tensor->desc.GetBHWCShape().c));
}
...
bool IsConvInt8KernelRecommendedForFullyConnectedOp(
    ...
    const ir::IrModel& graph, const ir::IrOp* fc_node) {
  const ir::IrTensor* src_tensor = graph.tensor(fc_node->inputs[0]);
  const int total_spatial_size = src_tensor->desc.GetBHWCShape().b *
                                 src_tensor->desc.GetBHWCShape().h *
                                 src_tensor->desc.GetBHWCShape().w;
...
SharedMemoryManager::GetInt4TensorDescExperimental(
  ...
  RETURN_IF_ERROR(RearrangeWeightsUInt4Packed(
...
SharedMemoryManager::GetInt2TensorDescExperimental(
  ...
  RETURN_IF_ERROR(RearrangeWeightsUInt2Packed(
      weights_u8, weights_desc, absl::MakeSpan(rearranged_weights),
      absl::MakeSpan(weights_sum_i->data),
...
absl::Status SharedMemoryManager::CalculateWeightsSumI(
    const TfLiteTensor& tflite_tensor, const ir::IrTensor* shared_const_tensor,
    Tensor<Linear, DataType::INT32>* weights_sum_i) {
  BHWC bhwc_shape = shared_const_tensor->desc.GetBHWCShape();
...
absl::Status SharedMemoryManager::TryRestoringSerializedTensor(
  ...
  RETURN_IF_ERROR(serialization_cache_->LookUp(
...
absl::Status SharedMemoryManager::TryStoringSerializedTensor(
  ...
  if (serialization_cache_ && serialization_cache_->IsReadyForInsert()) {
    RETURN_IF_ERROR(serialization_cache_->Insert(
        global_tensor_id,
...
absl::Status SharedMemoryManager::CreateQuantizedInt8WeightsTensor(
    const ir::IrTensorId& shared_tensor_id, const uint32_t global_tensor_id,
  ...
  const ir::IrTensor* shared_const_tensor = graph_.tensor(shared_tensor_id);
  // NOTE: GraphFloat32 set an intermediate UINT8 dtype on the shared constant
  // tensor here, but it was always overwritten before the function returned. We
  // instead update the descriptor in place (shape and dtype together) at the
  // terminal points below, matching GraphFloat32's in-place mutation.
  BHWC bhwc_shape = shared_const_tensor->desc.GetBHWCShape();
  OHWI shape(bhwc_shape.b, bhwc_shape.h, bhwc_shape.w, bhwc_shape.c);
  WeightsDescription weights_desc = GetFullyConnectedInt8WeightsDesc(
      gpu_info_, shape,
  ...
  if (!serialized_tensor_found) {
    if (has_prepacked_tflite_tensors_) {
      ...
      ASSIGN_OR_RETURN(shared_tensor.weights,
      ...
      if (weights_manager_) {
        weights_manager_->RegisterWeightsConversion(
            {static_cast<uint32_t>(shared_tensor_id)}, weights_desc, shape,
            DataType::INT8, tflite_tensor.data.int8);

        auto weights_i8_tensor_desc =
            GetTensorDescriptorsForWeightsLayout(shape, weights_desc)[0];
        {
          TensorDescriptor new_desc = shared_const_tensor->desc;
          new_desc.SetBHWCShape(weights_i8_tensor_desc.GetBHWCShape());
          new_desc.SetDataType(weights_i8_tensor_desc.GetDataType());
          graph_.GetMutableTensor(shared_const_tensor->id)->desc = new_desc;
        }
        return absl::OkStatus();
      }
      TensorDescriptor weights_i8_td = GetInt8TensorDesc(
          ...
          is_weight_sum_i_required, weights_sum_i);
      RETURN_IF_ERROR(create_tensor_func_(
  ...
  {
    TensorDescriptor new_desc = shared_const_tensor->desc;
    new_desc.SetBHWCShape(
        shared_tensor.weights->GetDescriptor().GetBHWCShape());
    new_desc.SetDataType(shared_tensor.weights->GetDescriptor().GetDataType());
    graph_.GetMutableTensor(shared_const_tensor->id)->desc = new_desc;
  }
  return absl::OkStatus();
}
  ...
  if (!serialized_tensor_found) {
    if (has_prepacked_tflite_tensors_) {
      ...
      if (weights_manager_ && shape.i % 4 == 0) {
        weights_manager_->RegisterWeightsConversion(
            {static_cast<uint32_t>(shared_tensor_id)}, weights_desc, shape,
            DataType::INT4, tflite_tensor.data.int8);

        auto weights_i4_tensor_desc =
            GetTensorDescriptorsForWeightsLayout(shape, weights_desc)[0];
        {
          TensorDescriptor new_desc = shared_const_tensor->desc;
          new_desc.SetBHWCShape(weights_i4_tensor_desc.GetBHWCShape());
          new_desc.SetDataType(weights_i4_tensor_desc.GetDataType());
          graph_.GetMutableTensor(shared_const_tensor->id)->desc = new_desc;
        }
        return absl::OkStatus();
      }
  ...
  if (!serialized_tensor_found) {
    if (has_prepacked_tflite_tensors_) {
      ...
      if (weights_manager_) {
        weights_manager_->RegisterWeightsConversion(
            {static_cast<uint32_t>(shared_tensor_id)}, weights_desc, shape,
            DataType::INT2, tflite_tensor.data.int8);

        auto weights_i2_tensor_desc =
            GetTensorDescriptorsForWeightsLayout(shape, weights_desc)[0];
        {
          TensorDescriptor new_desc = shared_const_tensor->desc;
          new_desc.SetBHWCShape(weights_i2_tensor_desc.GetBHWCShape());
          new_desc.SetDataType(weights_i2_tensor_desc.GetDataType());
          graph_.GetMutableTensor(shared_const_tensor->id)->desc = new_desc;
        }
        return absl::OkStatus();
      }
...
absl::Status SharedMemoryManager::CreateAffineQuantizationParams(
  ...
  Linear scale_shape(quant_params->scale->size);
  RewriteDenormalScales(gpu_info_, create_info_, quant_params->scale->data,
                        quant_params->scale->size);

  shared_tensor->scale_global_tensor_id = ++next_const_tensor_id_;
  ASSIGN_OR_RETURN(
      ir::IrTensorId scale_value_id,
      AddInputWithData(shared_tensor->scale_global_tensor_id.value(),
                       scale_shape, *fc_node, quant_params->scale->data,
                       data_type));
  ...
  // If we have more zero_points, copy them.
  if (quant_params->zero_point->size != quant_params->scale->size) {
    zero_points.resize(quant_params->scale->size, zero_points[0]);
  }
  Linear zero_point_shape(zero_points.size());
  ASSIGN_OR_RETURN(
      ir::IrTensorId zero_point_value_id,
  ...
  if (is_weight_sum_i_required) {
    ...
    ir::IrTensorId weights_sum_i_value_id;
    ...
    if (serialized_tensor_found) {
      ...
      weights_manager_->RegisterWeightsSumIConversion(
          {static_cast<uint32_t>(weights_sum_i_value_id)}, weights_shape,
          src_data_type, tflite_tensor.data.int8);
    } else {
      auto* weights_sum_i_data = shared_tensor->weights_sum_i.data.empty()
                                     ? nullptr
                                     : shared_tensor->weights_sum_i.data.data();
      absl::StatusOr<ir::IrTensorId> status_or_value_id = AddInputWithData(
...
absl::Status SharedMemoryManager::CreateBlockwiseQuantizationParams(
  ...
  shared_tensor->scale_global_tensor_id = ++next_const_tensor_id_;
  ASSIGN_OR_RETURN(ir::IrTensorId scale_value_id,
                   AddScaleNodeWithData(
                       shared_tensor->scale_global_tensor_id.value(), *fc_node,
                       scale_data, data_type, num_channels, num_blocks));
  shared_tensor = &buffer_id_to_spatial_tensor_.at(global_tensor_id);
  (*external_tensors)[scale_value_id] =
      GlobalId::BuildParamId(shared_tensor->scale_global_tensor_id.value());
...
absl::Status SharedMemoryManager::CreateQuantizedTensorWithScaleAndZeroPoint(
  ...
  const ir::IrTensor* shared_const_tensor = graph_.tensor(shared_tensor_id);
  OHWI weights_shape(shared_const_tensor->desc.GetBHWCShape().b,
                     shared_const_tensor->desc.GetBHWCShape().h,
                     shared_const_tensor->desc.GetBHWCShape().w,
                     shared_const_tensor->desc.GetBHWCShape().c);
  std::vector<ir::IrOp*> weight_consumers =
      graph_.FindConsumers(shared_tensor_id);
  if (weight_consumers.size() != 1) {
    return absl::InternalError(
        "Expected to have only one shared weight consumer.");
  }
  ir::IrOp* fc_node = weight_consumers[0];
  DataType data_type = data_type_;
  if (data_type_ == DataType::FLOAT32) {
    if (!fc_node->inputs.empty()) {
      DataType input_type =
          graph_.tensor(fc_node->inputs[0])->desc.GetDataType();
  ...
  if (fc_node->name == ToString(OperationType::FULLY_CONNECTED_INT8) ||
      fc_node->name == ToString(OperationType::FULLY_CONNECTED_INT4)) {
    ...
    // constants like weights_sum_i) on Gpu, we always compute weights_sum_i for
    // quantized FullyConnected ops if weights_sum_i is supported.
    //
    // This improves performance for preparing weights on Gpu, because it puts
    // weight-repacking and weights_sum_i computations for LLM's shared weights
    // into one single InferenceContext:
    ...
    //   eliminates the latency to build and execute the second InferenceContext
    //   for LLMs' weights preparation, as the first InferenceContext for
    //   weights preparation already covers all the weights preparation
    //   requests.
    const bool is_conv_int8_recommended =
        IsConvInt8KernelRecommendedForFullyConnectedOp(create_info_, gpu_info_,
                                                       graph_, fc_node) ||
        weights_manager_;
  ...
  if (tflite_tensor.type == TfLiteType::kTfLiteInt8) {
    RETURN_IF_ERROR(CreateQuantizedInt8WeightsTensor(
        shared_tensor_id, global_tensor_id, tflite_tensor, *shared_tensor,
        is_weight_sum_i_required, &shared_tensor->weights_sum_i,
        external_tensors));
  } else if (tflite_tensor.type == TfLiteType::kTfLiteInt4) {
    RETURN_IF_ERROR(CreateQuantizedInt4WeightsTensor(
        shared_tensor_id, global_tensor_id, tflite_tensor, *shared_tensor,
        is_weight_sum_i_required, &shared_tensor->weights_sum_i,
        external_tensors));
  } else if (tflite_tensor.type == TfLiteType::kTfLiteInt2) {
    RETURN_IF_ERROR(CreateQuantizedInt2WeightsTensor(
        shared_tensor_id, global_tensor_id, tflite_tensor, *shared_tensor,
        is_weight_sum_i_required, &shared_tensor->weights_sum_i,
        external_tensors));
  }
...
absl::StatusOr<ir::IrTensorId> SharedMemoryManager::AddInputWithData(
    uint32_t global_tensor_id, const Linear& shape,
    const ir::IrOp& consumer_node, const InputDataType* data,
    DataType data_type) {
  ...
  ASSIGN_OR_RETURN(ir::IrTensorId local_value_id,
  ...
  tensor_desc.SetBHWCShape(graph_.tensor(local_value_id)->desc.GetBHWCShape());
  RETURN_IF_ERROR(tensor_desc.UpdateToSupportedStorageType(
      gpu_info_, tensor_desc.GetBHWCShape()));

  tensor_desc.UploadData(data);
  RETURN_IF_ERROR(create_tensor_func_(tensor_desc, /*page_adjusted_offset=*/0,
                                      /*release_data_callback=*/nullptr,
                                      tensor_it->second.weights));
...
template <typename InputDataType>
absl::StatusOr<ir::IrTensorId> SharedMemoryManager::AddScaleNodeWithData(
...
absl::StatusOr<ir::IrTensorId> SharedMemoryManager::AddInputNode(
    uint32_t global_tensor_id, const BHWC& shape, const ir::IrOp& consumer_node,
    DataType data_type) {
  ir::IrTensor* value = graph_.add_tensor(ml_drift::TensorDescriptor{});

  value->desc.SetDataType(data_type);
  value->desc.SetBHWCShape(shape);

  graph_.AddConsumer(value->id, consumer_node.id);
  return value->id;
}
...
absl::Status SharedMemoryManager::RetrieveTensorWithScaleAndZeroPoint(
  ...
  if (data_type_ == DataType::FLOAT32) {
    if (!fc_node->inputs.empty()) {
      data_type = graph_.tensor(fc_node->inputs[0])->desc.GetDataType();
    }
  }
  bool is_weight_sum_i_required = false;
  if (fc_node->name == ToString(OperationType::FULLY_CONNECTED_INT8) ||
    ...
    if (is_weight_sum_i_required) {
      if (shared_tensor.weights_sum_i_global_tensor_id.has_value()) {
        ...
        if (weights_manager_) {
          ...
          OHWI weights_shape = OHWI(shared_const_tensor->desc.GetBHWCShape().b,
                                    shared_const_tensor->desc.GetBHWCShape().h,
                                    shared_const_tensor->desc.GetBHWCShape().w,
                                    shared_const_tensor->desc.GetBHWCShape().c);
          DataType src_data_type = DataType::INT4;
  ...
  if (weights_manager_) {
    ...
    {
      TensorDescriptor new_desc = shared_const_tensor->desc;
      new_desc.SetBHWCShape(weights_int_tensor_desc.GetBHWCShape());
      new_desc.SetDataType(weights_int_tensor_desc.GetDataType());
      graph_.GetMutableTensor(shared_const_tensor->id)->desc = new_desc;
    }
  } else {
    ...
    {
      TensorDescriptor new_desc = shared_const_tensor->desc;
      new_desc.SetBHWCShape(BHWC(weights->Batch(), weights->Height(),
                                 weights->Width(), weights->Channels()));
      new_desc.SetDataType(weights->GetDescriptor().GetDataType());
      graph_.GetMutableTensor(shared_const_tensor->id)->desc = new_desc;
    }
  }
  return absl::OkStatus();
...
absl::Status SharedMemoryManager::CreateSharedTensor(
  ...
  RETURN_IF_ERROR(MaybeBindTensorData(shared_tflite_tensor, tensor));
  ...
  DataType data_type = data_type_;
  if (data_type_ == DataType::FLOAT32) {
    std::vector<ir::IrOp*> consumers = graph_.FindConsumers(shared_tensor_id);
    if (consumers.size() == 1 && !consumers[0]->inputs.empty()) {
      data_type = DataType::FLOAT16;
      data_type = graph_.tensor(consumers[0]->inputs[0])->desc.GetDataType();
    }
  }
  ...
  Layout layout =
      shared_tflite_tensor.layout.has_value()
          ? shared_tflite_tensor.layout.value()
          : (graph_.tensor(shared_tensor_id)->desc.GetBHWCShape().b == 1
                 ? Layout::HWC
                 : Layout::BHWC);
  BHWC shape =
      layout == Layout::LINEAR
          ? BHWC(1, 1, 1,
                 graph_.tensor(shared_tensor_id)->desc.GetBHWCShape().c)
          : graph_.tensor(shared_tensor_id)->desc.GetBHWCShape();
  tensor_desc =
  ...
  RETURN_IF_ERROR(tensor_desc.UpdateToSupportedStorageType(
      gpu_info_, tensor_desc.GetBHWCShape()));
  // Update the shared constant's descriptor in place with the fully-formed
  // descriptor, matching GraphFloat32.
  graph_.GetMutableTensor(shared_tensor_id)->desc = tensor_desc;
...
//
// About IDs:
// * shared_tensor_id is the tensor ID in ir::IrModel, only valid for the
// current subgraph.
...
// * local_to_global_id_map is used to map the local ir::IrModel tensor ID (eg.
...
absl::Status SharedMemoryManager::RegisterExternalConstantTensors(
  ...
  if (new_spatial_tensor_inserted) {
    RETURN_IF_ERROR(CreateSharedTensor(shared_tensor_id, shared_tflite_tensor,
                                       tensor_it->second.weights,
    ...
    if (madvise_original_tensors_) {
      ...
      RETURN_IF_ERROR(DiscardTensorData(shared_tflite_tensor));
    }
  } else {
    RETURN_IF_ERROR(MaybeBindTensorData(shared_tflite_tensor, tensor));
  }
  if (!new_spatial_tensor_inserted &&
    ...
    if (!shared_tflite_tensor.dequant_forced) {
      RETURN_IF_ERROR(RetrieveTensorWithScaleAndZeroPoint(
          shared_tensor_id, shared_tflite_tensor.global_id, tensor,
          &local_to_global_id_map));
    }
    if (shared_tflite_tensor.dequant_forced) {
...
SharedMemoryManager::CreatePrepackedWeightsTensorFromTfliteTensor(
  ...
  UnownedDataTensorDescriptor unowned_desc(tensor_desc, data);
  std::unique_ptr<GpuSpatialTensor> tensor;
  RETURN_IF_ERROR(create_tensor_func_(unowned_desc, /*page_adjusted_offset=*/0,
                                      /*release_data_callback=*/nullptr,
                                      tensor));
  return tensor;
}
