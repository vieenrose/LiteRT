...
#import "third_party/odml/litert/ml_drift/delegate/shared_memory_manager/ir/shared_memory_manager_metal.h"
...
- (void)testSharedMemoryManagerMetalGetNonExistingExternalTensor {
  ...
  ml_drift::ir::TensorIdToSharedTensorMap buffer_id_to_spatial_tensor;
  ml_drift::ir::TensorIdToSharedTensorMap quant_param_tensors;
  ...
  auto metal_shared_memory_manager = ml_drift::ir::MakeSharedMemoryManagerMetal(
  ...
  ml_drift::ir::SharedMemoryManager::GlobalId non_existing_global_id =
      ml_drift::ir::SharedMemoryManager::GlobalId::BuildSourceId(0);
...
- (void)testCreateExternalFloatTensorF16FromF32 {
  ...
  ml_drift::ir::IrTensor* input = graph.add_tensor(ml_drift::TensorDescriptor{});
  ml_drift::ir::IrTensor* output = graph.add_tensor(ml_drift::TensorDescriptor{});
  input->consumers.insert(op->id);
  op->inputs.push_back(input->id);
  output->producer = op->id;
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(ml_drift::BHWC(1, 1, 1, 10));

  TfLiteContext context;
  id<MTLDevice> device = MTLCreateSystemDefaultDevice();
  ml_drift::metal::MetalDevice metal_device(device);
  auto manager = ml_drift::ir::MakeSharedMemoryManagerMetal(
      &metal_device, create_info, graph, &context, buffer_id_to_spatial_tensor, quant_param_tensors,
  ...
  absl::flat_hash_map<ml_drift::ir::IrTensorId, ml_drift::ir::SharedMemoryManager::GlobalId>
...
- (void)testCreateExternalFloatTensorF32FromF16 {
  ...
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(ml_drift::BHWC(1, 1, 1, 10));
  input->desc.SetDataType(ml_drift::DataType::FLOAT16);
...
- (void)testCreateExternalFloatTensorF32FromF32 {
  ...
  op->outputs.push_back(output->id);
  input->desc.SetBHWCShape(ml_drift::BHWC(1, 1, 1, 10));
  input->desc.SetDataType(ml_drift::DataType::FLOAT32);
...
- (void)runBlockwiseQuantizationTestWithZpType:(TfLiteType)zpType
  ...
  ml_drift::ir::IrTensor* weights_val = graph.add_tensor(ml_drift::TensorDescriptor{});
  weights_val->consumers.insert(op->id);
  op->inputs.push_back(weights_val->id);
  weights_val->desc.SetBHWCShape(ml_drift::BHWC(100, 1, 1, 64));
