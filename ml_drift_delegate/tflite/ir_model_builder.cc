  ...
  void ProcessTensor(
    ...
    if (::tflite::IsConstantTensor(&context_.tensors[tfl_tensor_id])) {
      if (tensor_to_buffer_id_map_) {
        if (auto it = tensor_to_buffer_id_map_->find(tfl_tensor_id);
            it != tensor_to_buffer_id_map_->end()) {
          is_shared = true;
          global_id = it->second;
        }
      }
      if (!is_shared && tensor_to_external_buffer_id_map_) {
    ...
    const auto ir_tensor_id = AddTensor(
        ...
        ::ml_drift::ir::BufferSource{is_shared, is_external, global_id});
