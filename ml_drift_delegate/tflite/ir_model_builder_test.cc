  ...
  void VerifySharedWeightsTensor(int weights_tensor_index,
    ...
    for (const auto& tensor : test_data_.ir_model.tensors()) {
      if (tensor->buffer_source.is_shared) {
        ...
        EXPECT_EQ(tensor->buffer_source.is_external, expected_is_external);
...
TEST_F(IrModelBuilderTest, PrefersInternalWhenTensorInBothMaps) {
  ...
  // The internal map is checked first, so the tensor is treated as an internal
  // (non-external) shared constant using the internal global id.
  VerifySharedWeightsTensor(kWeightsTensorIndex, kInternalGlobalId,
                            /*expected_is_external=*/false);
}
